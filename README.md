...
> **Why?** When you purchase a number on nexusai, your original balance `($20)` isn't getting deducted from your twilio account since the number was purchased from nexusai in `DEVELOPMENT_MODE` as a result that number purchased isn't getting provisioned to your twilio account, which is why you first need to purchase the number directly from twilio. So, to make sure the number is linked to your account, you would need to update the `used_phone_numbers` table with the `phone` field with the number you purchased directly from twilio. your balance is only getting deducted when you either (1) puchase a number directly from twilio or (2) when you make a voice call or send sms.
